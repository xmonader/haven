...
- **Done:** M0 skeleton. **M1** — object model (`internal/object`: blob/tree/commit, content-addressed store, nested tree build/flatten), `internal/hash`, `internal/ref` (CRUD + visibility), `internal/index` (staging), `internal/workspace` (scan), `internal/config` (author). Commands: `init config add commit status log`. End-to-end + unit tests green, vet clean.
  ...
  **Hardening pass** — body-bound request signatures (sign method+path+time+sha256(body); closes ref-tamper/replay), per-actor reachable-object cache on the server (O(repo) once, not per object), drift baseline armed at first commit, TLS serving (`--tls-cert/--tls-key`), symlink tracking (mode 120000), rename-aware three-way merge, porcelain `reset`/`restore`/`tag`, and unit tests for remote/index/config/lock/identity. 13 test packages green, vet clean.
- **Next:** all milestones (M0–M8) delivered and hardened. Deferred by choice (feature scope, not gaps): delta/pack compression (SQLite already pages), and rebase/cherry-pick/bisect/stash/octopus-merge.
- **Blocked:** nothing.
